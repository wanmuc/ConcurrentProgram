...
void usage() {
  cout << "IoUringProactorSingleProcessCoroutineHSHA1 -ip 0.0.0.0 -port 1688" << endl;
  cout << "options:" << endl;
...
void Producer(MyCoroutine::Channel<IoURing::Request> &channel, IoURing::Request *request) { channel.Send(request); }
...
int main(int argc, char *argv[]) {
  ...
  MyCoroutine::Channel<IoURing::Request> channel(schedule, 3000);  // channel初始化，分配3000的缓存
  for (int i = 0; i < 3000; i++) {
    int cid = schedule.CoroutineCreate(Consumer, ref(schedule), ref(channel), ref(ring));
