...
void usage() {
  cout << "EpollReactorProcessPoolHSHA -ip 0.0.0.0 -port 1688 -worker 5 -io 3 " << endl;
  cout << "options:" << endl;
  cout << "    -h,--help      print usage" << endl;
  cout << "    -ip,--ip       listen ip" << endl;
  cout << "    -port,--port   listen port" << endl;
  cout << "    -worker,--worker   worker process count" << endl;
...
int main(int argc, char *argv[]) {
  string ip;
  int64_t port;
  int64_t worker_process_count;
  int64_t io_thread_count;
  bool is_main_read;
  CmdLine::StrOptRequired(&ip, "ip");
  CmdLine::Int64OptRequired(&port, "port");
  CmdLine::Int64OptRequired(&worker_process_count, "worker");
  CmdLine::Int64OptRequired(&io_thread_count, "io");
  CmdLine::SetUsage(usage);
  CmdLine::Parse(argc, argv);
  worker_process_count = worker_process_count > GetNProcs() ? GetNProcs() : worker_process_count;
  io_thread_count = io_thread_count > GetNProcs() ? GetNProcs() : io_thread_count;
  // TODO
  int v_op_unix_socket;                                          // 用于发送 v 信号的 unix socket
  int p_op_uinx_socket;                                          // 用于接收 p 信号的 unix socket
  createWorkerProcess(send_fd_unix_sockets, sub_reactor_count);  // 创建SubReactor进程
  createIoThread(ip, port, is_main_read, main_reactor_count, send_fd_unix_sockets);  // 创建MainRector进程
